...
export default class EditEmployerOverviewPage {
    
        private readonly elements = {
            companyRegisteredName: element(by.css('input[formcontrolname="name"]')),
            ...
            public async enterCompanyRegisteredName(){
                await browser.wait(ExpectedConditions.elementToBeClickable(this.elements.companyRegisteredName), 10000);
                await this.elements.companyRegisteredName.clear();
                await this.elements.companyRegisteredName.sendKeys(`Automation Edit Test: ${new Date().toUTCString()}`);
            ...
            public async editEmployerOverview(){
                await this.enterCompanyRegisteredName();
