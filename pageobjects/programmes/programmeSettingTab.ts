import { element, by, browser, until, ExpectedConditions, protractor } from "protractor/built";
...
export default class ProgrammeSettingsTab {
    ...
    public async editSector() {
        ...
        browser.wait(ExpectedConditions.elementToBeClickable(this.elements.sectorsDropdownResults.get(0)), 10000);
        await this.elements.sectorsDropdownResults.count().then((numberOfItems) => {
            return Math.floor(Math.random() * numberOfItems) + 1;
        })
        
        .then((randomNumber) => {
            this.elements.sectorsDropdownResults.get(randomNumber).click();
        });
    }
